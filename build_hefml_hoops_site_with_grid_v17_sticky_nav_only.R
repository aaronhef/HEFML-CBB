...
build_hefml_hoops_site <- function(pred_data,
  ...
  # --- odds/lines helpers -------------------------------------------
  moneyline_to_implied <- function(ml) {
    ml <- suppressWarnings(as.numeric(ml))
    out <- rep(NA_real_, length(ml))
    pos <- !is.na(ml) & ml > 0
    neg <- !is.na(ml) & ml < 0
    out[pos] <- 100 / (ml[pos] + 100)
    out[neg] <- (-ml[neg]) / ((-ml[neg]) + 100)
    out
  }

  # Pull DraftKings (Draft Kings) lines from a CFBD-style /lines endpoint.
  # Uses env vars:
  #   HEFML_LINES_BASE (default https://api.collegefootballdata.com)
  #   CFBD_API_KEY (optional; if present uses Authorization: Bearer <key>)
  get_dk_lines <- function(season, season_type = "regular") {
    base_url <- Sys.getenv("HEFML_LINES_BASE", "https://api.collegefootballdata.com")
    api_key  <- Sys.getenv("CFBD_API_KEY", "")

    if (!requireNamespace("httr", quietly = TRUE) || !requireNamespace("jsonlite", quietly = TRUE)) {
      warning("httr/jsonlite not installed; skipping lines pull")
      return(NULL)
    }

    url <- paste0(base_url, "/lines")
    q <- list(season = as.integer(season), seasonType = season_type)
    ...
    resp <- try(httr::GET(url, query = q, headers), silent = TRUE)
    if (inherits(resp, "try-error")) return(NULL)
    if (httr::status_code(resp) >= 400) return(NULL)

    raw <- httr::content(resp, as = "text", encoding = "UTF-8")
    x <- try(jsonlite::fromJSON(raw, flatten = TRUE), silent = TRUE)
    if (inherits(x, "try-error") || is.null(x) || nrow(x) == 0) return(NULL)

    # Normalize nested 'lines' list
    if (!("lines" %in% names(x))) return(NULL)

    # If flatten didn't fully unnest, do it manually.
    if (is.list(x$lines)) {
      lens <- lengths(x$lines)
      if (all(lens == 0)) return(NULL)
      idx <- rep(seq_len(nrow(x)), lens)
      y <- x[idx, setdiff(names(x), "lines"), drop = FALSE]
      lines_df <- do.call(rbind, lapply(x$lines, function(z) if (is.null(z) || length(z) == 0) NULL else as.data.frame(z, stringsAsFactors = FALSE)))
      if (is.null(lines_df) || nrow(lines_df) == 0) return(NULL)
      lines_df <- lines_df[rep(seq_len(nrow(lines_df)), 1), , drop = FALSE]
      x <- cbind(y, lines_df)
    }

    # provider filter (DraftKings vs Draft Kings)
    prov <- tolower(gsub("[^a-z]", "", x$lines.provider))
    keep <- prov %in% c("draftkings", "draftking")
    x <- x[keep, , drop = FALSE]
    if (nrow(x) == 0) return(NULL)

    # standardize col names we care about
    out <- data.frame(
      gameId        = suppressWarnings(as.integer(x$gameId)),
      startDate     = as.character(x$startDate),
      homeTeam      = as.character(x$homeTeam),
      awayTeam      = as.character(x$awayTeam),
      spreadOpen    = suppressWarnings(as.numeric(x$lines.spreadOpen)),
      spread        = suppressWarnings(as.numeric(x$lines.spread)),
      homeMoneyline = suppressWarnings(as.numeric(x$lines.homeMoneyline)),
      awayMoneyline = suppressWarnings(as.numeric(x$lines.awayMoneyline)),
      stringsAsFactors = FALSE
    )
    out
  }

  build_sharps_text <- function(pred_today, lines_today) {
    # pred_today must include: homeTeam, awayTeam, spread_pred (model spread from home perspective)
    # lines_today must include: spreadOpen, spread, homeMoneyline, awayMoneyline

    # If your model uses "positive = home win margin", then "spread_pred" should already be home spread
    # (negative means home favored). If your pipeline's spread_pred is margin, convert before calling.

    # Join by home/away names if ids aren't present
    if (!is.null(lines_today) && nrow(lines_today) > 0) {
      j <- merge(
        pred_today, lines_today,
        by = c("homeTeam", "awayTeam"),
        all.x = TRUE
      )
    } else {
      j <- pred_today
      j$spreadOpen <- NA_real_; j$spread <- NA_real_
      j$homeMoneyline <- NA_real_; j$awayMoneyline <- NA_real_
    }

    # line moves list
    moved <- j[!is.na(j$spreadOpen) & !is.na(j$spread) & j$spreadOpen != j$spread, , drop = FALSE]
    move_lines <- character(0)
    if (nrow(moved) > 0) {
      # show biggest moves first
      moved$move <- moved$spread - moved$spreadOpen
      moved <- moved[order(abs(moved$move), decreasing = TRUE), , drop = FALSE]
      move_lines <- sprintf("• %s vs %s: open %s → now %s (%+.1f)",
                            moved$awayTeam, moved$homeTeam,
                            moved$spreadOpen, moved$spread, moved$move)
      # cap
      if (length(move_lines) > 10) move_lines <- move_lines[1:10]
    }

    # spread value picks (use model spread_pred vs current spread)
    # Convention assumed: spread is from HOME perspective (negative = home favored).
    picks_spread <- character(0)
    if ("spread_pred" %in% names(j) && any(!is.na(j$spread)) && any(!is.na(j$spread_pred))) {
      j$edge_spread <- j$spread - j$spread_pred
      # If edge_spread is positive, market gives home more points than model -> home value.
      # If negative, away value.
      j$bet_side <- ifelse(j$edge_spread >= 0, "HOME", "AWAY")
      j$edge_abs <- abs(j$edge_spread)
      cand <- j[!is.na(j$edge_abs), , drop = FALSE]
      cand <- cand[order(cand$edge_abs, decreasing = TRUE), , drop = FALSE]
      if (nrow(cand) > 0) {
        top <- head(cand, 5)
        picks_spread <- sprintf("• %s %s (model %s vs DK %s, edge %+.1f)",
                                ifelse(top$bet_side=="HOME", top$homeTeam, top$awayTeam),
                                ifelse(top$bet_side=="HOME", top$spread, -top$spread),
                                top$spread_pred, top$spread, top$edge_spread)
      }
    }

    # ML value picks (compare model money_prob to implied from DK moneylines)
    picks_ml <- character(0)
    if ("money_prob" %in% names(j) && any(!is.na(j$homeMoneyline)) && any(!is.na(j$money_prob))) {
      j$home_imp <- moneyline_to_implied(j$homeMoneyline)
      j$away_imp <- moneyline_to_implied(j$awayMoneyline)
      # money_prob is % home win
      j$home_model <- suppressWarnings(as.numeric(j$money_prob))/100
      j$away_model <- 1 - j$home_model
      j$home_edge <- j$home_model - j$home_imp
      j$away_edge <- j$away_model - j$away_imp

      ml_rows <- do.call(rbind, lapply(seq_len(nrow(j)), function(i){
        data.frame(
          game = paste(j$awayTeam[i], "at", j$homeTeam[i]),
          team = c(j$homeTeam[i], j$awayTeam[i]),
          ml   = c(j$homeMoneyline[i], j$awayMoneyline[i]),
          edge = c(j$home_edge[i], j$away_edge[i]),
          stringsAsFactors = FALSE
        )
      }))
      ml_rows <- ml_rows[!is.na(ml_rows$edge) & !is.na(ml_rows$ml), , drop = FALSE]
      ml_rows <- ml_rows[order(ml_rows$edge, decreasing = TRUE), , drop = FALSE]
      if (nrow(ml_rows) > 0) {
        topm <- head(ml_rows, 5)
        picks_ml <- sprintf("• %s %s (edge %+0.1f%%)", topm$team, topm$ml, 100*topm$edge)
      }
    }

    txt <- c()
    txt <- c(txt, "LINE MOVES (DK spread open → current)")
    if (length(move_lines) == 0) txt <- c(txt, "• No DK spread moves found (or no lines available).")
    else txt <- c(txt, move_lines)

    txt <- c(txt, "", "GOOD BETS (vs model)")
    txt <- c(txt, "SPREAD:")
    if (length(picks_spread) == 0) txt <- c(txt, "• (No spread edges computed.)")
    else txt <- c(txt, picks_spread)

    txt <- c(txt, "", "ML:")
    if (length(picks_ml) == 0) txt <- c(txt, "• (No ML edges computed.)")
    else txt <- c(txt, picks_ml)
...
.sticky-right{display:flex; align-items:center; gap:8px; flex-wrap:wrap; justify-content:flex-end}
.navbtn, .actionbtn{
...
.navbtn:active, .actionbtn:active{transform:scale(.98)}
.badge{
...
.gcard{cursor:default;-webkit-tap-highlight-color:rgba(0,0,0,0);}
...
/* HefML Bot + Sharps box */
.hefml-bot-row{display:flex;gap:12px;align-items:flex-start;margin:12px 0 6px 0}
.hefml-bot{flex:0 0 auto;width:84px;height:84px}
.hefml-bubble{
  position:relative;
  flex:1 1 auto;
  padding:10px 12px;
...
.hefml-bubble:after{
  ...
  border-style:solid;
  border-color:transparent rgba(0,0,0,0.28) transparent transparent;
}
.hefml-bubble pre{margin:0;white-space:pre-wrap;font-size:13px;line-height:1.25}
.hefml-bubble .title{font-weight:800;margin-bottom:6px;letter-spacing:.02em}
</style>
<script>
  ...
  <div class="container">
    <div class="sticky-inner">
      <div class="sticky-left">
        <div class="title">
          <svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true"><path d="M6 12c0 3 2.5 5 6 5s6-2 6-5-2.5-5-6-5-6 2-6 5Z" fill="#fbbf24"/><path d="M6 12c0 3 2.5 5 6 5s6-2 6-5-2.5-5-6-5-6 2-6 5Z" fill="#60a5fa" opacity=".55"/><path d="M8 13c1 1 2.5 1.5 4 1.5S15 14 16 13" stroke="#2b1406" stroke-width="1.5" fill="none" stroke-linecap="round"/></svg>
          <span>HefML Hoops</span>
        </div>
      </div>
      <div class="sticky-right">
        <a class="navbtn" href="#top">↑ Top</a>
        <a class="navbtn" href="#games">Games</a>
        <a class="navbtn" href="#teams">Teams</a>
      </div>
    </div>
  </div>
</div>
...
'
  ...
  build_card <- function(row) {
    ...
    paste0(
      '
<div class="', gcard_cls, '"',
      ' data-pick-team="', if (is.na(mid)) '' else if (mid >= 0) home else away, '"',
      ' data-pick-spread="', if (is.na(mid)) '' else format(round(mid, 1), nsmall = 1), '"',
      '>',
      '\n  <div class="row1">',
...
# ------------------------------------------------------------
      ...
      if (nrow(fav) > 0) {
        ...
        # --- NEW: grid axes use team power (x) and conference power (y) ----------
        # Team power is derived from home_power in predictions_2026 (and away_power if present).
        # Support both snake_case (home_team) and camelCase (homeTeam) column conventions.

        home_team_col <- if ("home_team" %in% names(pred_data)) "home_team" else if ("homeTeam" %in% names(pred_data)) "homeTeam" else NA_character_
        away_team_col <- if ("away_team" %in% names(pred_data)) "away_team" else if ("awayTeam" %in% names(pred_data)) "awayTeam" else NA_character_

        # ---- team power map (x axis) ----
        power_map <- NULL
        hp <- NULL; ap <- NULL
        if (!is.na(home_team_col) && "home_power" %in% names(pred_data)) {
          hp <- tapply(as.numeric(pred_data$home_power),
                       as.character(pred_data[[home_team_col]]),
                       mean, na.rm = TRUE)
        }
        if (!is.na(away_team_col) && "away_power" %in% names(pred_data)) {
          ap <- tapply(as.numeric(pred_data$away_power),
                       as.character(pred_data[[away_team_col]]),
                       mean, na.rm = TRUE)
        }
        if (!is.null(hp) || !is.null(ap)) {
          all_teams <- unique(c(names(hp), names(ap)))
          pow <- numeric(length(all_teams)); names(pow) <- all_teams
          for (tm0 in all_teams) {
            vals <- c(if (!is.null(hp) && tm0 %in% names(hp)) hp[[tm0]] else NA_real_,
                      if (!is.null(ap) && tm0 %in% names(ap)) ap[[tm0]] else NA_real_)
            pow[[tm0]] <- mean(vals, na.rm = TRUE)
          }
          power_map <- pow
        }

        # ---- conference power map (y axis) ----
        conf_power_map <- NULL
        hcp <- NULL; acp <- NULL
        if (!is.na(home_team_col) && "home_conf_power" %in% names(pred_data)) {
          hcp <- tapply(as.numeric(pred_data$home_conf_power),
                        as.character(pred_data[[home_team_col]]),
                        mean, na.rm = TRUE)
        }
        if (!is.na(away_team_col) && "away_conf_power" %in% names(pred_data)) {
          acp <- tapply(as.numeric(pred_data$away_conf_power),
                        as.character(pred_data[[away_team_col]]),
                        mean, na.rm = TRUE)
        }
        if (!is.null(hcp) || !is.null(acp)) {
          all_teams2 <- unique(c(names(hcp), names(acp)))
          cpm <- numeric(length(all_teams2)); names(cpm) <- all_teams2
          for (tm0 in all_teams2) {
            vals <- c(if (!is.null(hcp) && tm0 %in% names(hcp)) hcp[[tm0]] else NA_real_,
                      if (!is.null(acp) && tm0 %in% names(acp)) acp[[tm0]] else NA_real_)
            cpm[[tm0]] <- mean(vals, na.rm = TRUE)
          }
          conf_power_map <- cpm
        }

        # If either axis is entirely missing, fall back to zeroes so we still render the grid.
        # (We still keep the original filtering by logo URL.)
        team_grid$team_power <- if (!is.null(power_map)) as.numeric(power_map[team_grid$team]) else NA_real_
        team_grid$team_conf_power <- if (!is.null(conf_power_map)) as.numeric(conf_power_map[team_grid$team]) else NA_real_
        ...
        if (nrow(team_grid) > 0) {
          ...
          xr <- range(team_grid$team_power, na.rm = TRUE)
          yr <- range(team_grid$team_conf_power,      na.rm = TRUE)
          ...
          if (diff(xr) == 0) {
            ...
            team_grid$x_pct <- pad + (team_grid$team_power - xr[1]) / diff(xr) * span
          }

          if (diff(yr) == 0) {
            team_grid$y_pct <- 50
          } else {
            team_grid$y_pct <- pad + (team_grid$team_conf_power - yr[1]) / diff(yr) * span
          }
          ...
          for (i in seq_len(nrow(team_grid))) {
            ...
            pow_val <- sprintf("%.2f", team_grid$team_power[i])
            conf_pow_val <- sprintf("%.2f", team_grid$team_conf_power[i])

            logo_divs[i] <- paste0(
              '<div class="team-logo" style="left:', x, '%;bottom:', y, '%;width:4%;height:auto;" ',
              'title="', tm,
              '&#10;Team power: ', pow_val,
              '&#10;Conf power: ', conf_pow_val,
              '&#10;Fav margin: ', margin,
          ...
          logo_section <- paste0(
            ...
            '\n        <div class="logo-axis-label x">Team power</div>',
            '\n        <div class="logo-axis-label y">Conference power \u2191</div>',
            '\n      </div>',
            '\n    </div>',
            '\n  </div></div>\n'
          )
  ...
  # --- HefML Bot "Sharps" box (model + DK lines) --------------------
  sharps_section <- ""
  if (!is.null(bot_text) && nzchar(bot_text)) {
    sharps_text <- html_escape(bot_text)

    # Cute pixel-ish robot as inline SVG
    bot_svg <- paste0(
      '<svg class="hefml-bot" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">',
      # outline/head
      '<rect x="2" y="2" width="12" height="11" rx="2" ry="2" fill="#1f2a37" stroke="rgba(255,255,255,0.25)" stroke-width="0.5"/>',
      # antenna
      '<rect x="7.5" y="0.8" width="1" height="2.2" fill="#b8d4ff"/><rect x="7" y="0.2" width="2" height="1" fill="#ffb86b"/>',
      # eyes
      '<rect x="5" y="6" width="2" height="2" fill="#b8d4ff"/><rect x="9" y="6" width="2" height="2" fill="#b8d4ff"/>',
      # mouth
      '<rect x="5" y="9.5" width="6" height="1.2" fill="rgba(255,255,255,0.55)"/>',
      # body stripe
      '<rect x="3.5" y="3.8" width="9" height="1" fill="rgba(255,255,255,0.15)"/>',
      '</svg>'
    )

    sharps_section <- paste0(
      '\n  <div class="card"><div class="inner">',
      '\n    <div class="hefml-bot-row">',
      '\n      ', bot_svg,
      '\n      <div class="hefml-bubble">',
      '\n        <div class="title">HefML Bot Insights</div>',
      '\n        <pre>', sharps_text, '</pre>',
      '\n      </div>',
      '\n    </div>',
      '\n  </div></div>\n'
